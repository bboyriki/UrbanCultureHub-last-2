...
import {
  ...
  Loader2, Plus, Eye, Trash2, Edit3, Copy, Sparkles, RefreshCw, Bot,
...
export default function AdminMarketing() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CampaignForm) => {
      ...
      const d = await r.json(); if (!r.ok) throw new Error(d.message || "Failed to create"); return d;
    },
    onSuccess: async (campaign, vars) => {
      ...
      if (vars.scheduleType === "now") { await sendMutation.mutateAsync({ id: campaign.id, force: true }); }
      else { toast({ title: "Campaign created" }); resetModal(); }
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<CampaignForm> }) => {
      ...
      const d = await r.json(); if (!r.ok) throw new Error(d.message || "Failed"); return d;
    },
  ...
  const postToSocial = async () => {
    ...
    try {
      if (socialPlatforms.linkedin && linkedinStatus?.connected) {
        ...
        if (r.ok) results.push("LinkedIn"); else throw new Error(d.error || "LinkedIn failed");
      }
      ...
      if (results.length > 0) { toast({ title: `Posted to ${results.join(" & ")}! 🚀` }); setSocialText(""); setIgImageUrl(""); }
      else toast({ title: "No platforms selected or connected", variant: "destructive" });
  ...
  return (
    <div className="space-y-4">
      {/* Header banner */}
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white p-5">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_rgba(255,107,0,0.2),transparent_60%)]" />
        <div className="relative flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Rocket className="w-5 h-5 text-orange-400" />
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-base">Marketing Hub</h2>
                <span className="text-[10px] font-bold bg-orange-500/20 text-orange-300 border border-orange-500/30 px-1.5 py-0.5 rounded-full">PRO</span>
              </div>
              <p className="text-white/40 text-[11px]">Email · Social · App Promotion</p>
            </div>
          </div>
          <Button size="sm" className="bg-orange-500 hover:bg-orange-600 text-white border-0 h-8" onClick={() => { resetModal(); setShowModal(true); }} data-testid="button-new-campaign">
        ...
        <div className="relative mt-4 grid grid-cols-5 gap-2">
          {[
            { label: "Total", value: stats?.totalCampaigns || 0, icon: Megaphone, color: "text-orange-400" },
            ...
            <div key={s.label} className="bg-white/5 backdrop-blur rounded-xl border border-white/10 p-2.5 flex items-center gap-2">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="h-9 bg-muted/60">
          <TabsTrigger value="campaigns" className="text-xs gap-1" data-testid="tab-campaigns">
            <Megaphone className="w-3.5 h-3.5" /> Campaigns
          </TabsTrigger>
          <TabsTrigger value="social" className="text-xs gap-1" data-testid="tab-social">
            <Share2 className="w-3.5 h-3.5" /> Social Blast
          </TabsTrigger>
          <TabsTrigger value="promote" className="text-xs gap-1" data-testid="tab-promote">
          ...
          <TabsTrigger value="audience" className="text-xs gap-1" data-testid="tab-audience">
        ...
        <TabsContent value="campaigns" className="mt-3 space-y-2">
          {isLoading ? (
            ...
            <div className="bg-white rounded-xl border border-border/60 overflow-hidden">
              {campaigns.map((c) => (
                <div key={c.id} className="px-4 py-3.5 border-b border-border/40 last:border-b-0 hover:bg-gray-50/50 transition-colors" data-testid={`campaign-row-${c.id}`}>
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm">{c.name}</span>
                        <StatusBadge status={c.status} />
                        <span className="text-[11px] text-muted-foreground bg-gray-100 px-1.5 py-0.5 rounded">{AUDIENCE_LABELS[c.targetAudience] || c.targetAudience}</span>
                      </div>
                      <p className="text-xs text-muted-foreground truncate max-w-md">Subject: {c.subject}</p>
                      <div className="flex items-center gap-3 text-[11px] text-muted-foreground flex-wrap">
                        {c.sentAt ? <span className="flex items-center gap-1"><CheckCircle className="w-3 h-3 text-green-500" /> Sent {new Date(c.sentAt).toLocaleDateString("nl-NL")}</span>
                          : c.scheduledAt ? <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-blue-500" /> Scheduled {new Date(c.scheduledAt).toLocaleDateString("nl-NL")}</span>
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setViewCampaign(c)}><Eye className="w-3.5 h-3.5" /></Button>
                      {(c.status === "draft" || c.status === "scheduled") && <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openEdit(c)}><Edit3 className="w-3.5 h-3.5" /></Button>}
                      <Button size="sm" variant="outline" className="h-7 px-2 text-xs text-amber-600 border-amber-200 hover:bg-amber-50" onClick={() => sendTestMutation.mutate(c.id)} disabled={sendTestMutation.isPending} data-testid={`button-test-${c.id}`}>
                      ...
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openDuplicate(c)}><Copy className="w-3.5 h-3.5" /></Button>
                      {c.status !== "sent" && <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-400 hover:text-red-600" onClick={() => deleteMutation.mutate(c.id)}><Trash2 className="w-3.5 h-3.5" /></Button>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className={`rounded-xl border p-3 flex items-center justify-between ${linkedinStatus?.connected ? "border-blue-200 bg-blue-50" : "border-border bg-muted/30"}`}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">{linkedinStatus?.connected ? (linkedinStatus.profileName || "Connected") : "Not connected"}</p>
                </div>
              </div>
              {linkedinStatus?.connected ? (
                <label className="flex items-center gap-1 cursor-pointer text-xs font-medium text-blue-700">
                ...
                <a href="/admin/linkedin"><Button size="sm" variant="outline" className="h-6 text-xs px-2">Connect</Button></a>
              )}
            </div>

            <div className={`rounded-xl border p-3 flex items-center justify-between ${igStatus?.connected ? "border-pink-200 bg-pink-50" : "border-border bg-muted/30"}`}>
              <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${igStatus?.connected ? "bg-gradient-to-br from-pink-500 to-purple-600" : "bg-muted"}`}>
                  <SiInstagram className={`w-3.5 h-3.5 ${igStatus?.connected ? "text-white" : "text-muted-foreground"}`} />
                </div>
                <div>
                  <p className="font-medium text-xs">Instagram</p>
                  <p className="text-[11px] text-muted-foreground">{igStatus?.connected ? `@${igStatus.username || "connected"}` : "Not connected"}</p>
                </div>
              </div>
              {igStatus?.connected ? (
                <label className="flex items-center gap-1 cursor-pointer text-xs font-medium text-pink-700">
                ...
                <a href="/admin/instagram"><Button size="sm" variant="outline" className="h-6 text-xs px-2">Connect</Button></a>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-border/60 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-semibold text-sm">Post Composer</span>
              </div>
            ...
            <Textarea
              placeholder="Write your post — or hit AI Generate. Add emojis, hashtags and your app link for best reach!"
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>{socialText.length} chars</span>
              {socialText.length > 0 && (
                <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => setSocialText(socialText + "\n\n🌐 urbanculturehub.nl")}>+ App link</Button>
              )}
            </div>
            {socialPlatforms.instagram && igStatus?.connected && (
              <div className="p-3 rounded-lg bg-pink-50 border border-pink-200 space-y-1.5">
                <p className="text-[11px] font-medium text-pink-700 flex items-center gap-1"><SiInstagram className="w-3 h-3" /> Instagram requires an image URL</p>
                <Input placeholder="https://yourimage.com/photo.jpg" value={igImageUrl} onChange={e => setIgImageUrl(e.target.value)} className="text-xs h-7" data-testid="input-ig-image-url" />
              </div>
            )}
            <Button
        ...
        {/* ── PROMOTE APP TAB ── */}
        <TabsContent value="promote" className="mt-3 space-y-3">
          {/* QR Codes */}
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-xl border border-border/60 p-4 space-y-3">
              <div className="flex items-center gap-1.5">
                <Globe className="w-4 h-4 text-orange-500" />
                <span className="font-semibold text-sm">Website QR</span>
              </div>
              <div className="flex justify-center p-3 bg-gray-50 rounded-xl">
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(APP_URL)}&bgcolor=ffffff&color=111111&margin=8`} alt="Website QR" className="w-36 h-36" data-testid="img-qr-website" />
              </div>
              <div className="flex gap-2">
                <CopyButton text={APP_URL} label="Copy URL" />
                <a href={`https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(APP_URL)}&bgcolor=ffffff&color=111111&margin=20`} target="_blank" rel="noreferrer" className="flex-1">
                  <Button size="sm" variant="outline" className="w-full h-7 text-xs gap-1" data-testid="button-download-qr"><Download className="w-3 h-3" /> Download</Button>
                </a>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-border/60 p-4 space-y-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-semibold text-sm">iOS App QR</span>
              </div>
              <div className="flex justify-center p-3 bg-gray-50 rounded-xl">
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(IOS_URL)}&bgcolor=ffffff&color=111111&margin=8`} alt="iOS App QR" className="w-36 h-36" data-testid="img-qr-ios" />
              </div>
              <div className="flex gap-2">
                <CopyButton text={IOS_URL} label="Copy URL" />
                <a href={`https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(IOS_URL)}&bgcolor=ffffff&color=111111&margin=20`} target="_blank" rel="noreferrer" className="flex-1">
                  <Button size="sm" variant="outline" className="w-full h-7 text-xs gap-1"><Download className="w-3 h-3" /> Download</Button>
                </a>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-border/60 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <a href={`mailto:?subject=Check out Urban Culture Hub&body=Check out: ${APP_URL}`} target="_blank" rel="noreferrer">
                <Button size="sm" variant="outline" className="w-full h-8 text-xs gap-1.5 hover:bg-muted" data-testid="button-share-email"><Mail className="w-3.5 h-3.5" /> Email</Button>
              </a>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="font-semibold text-sm">Promo Copy Kit</span>
              <span className="text-xs text-muted-foreground ml-1">— ready-to-post captions</span>
            </div>
            <div className="space-y-2">
              {PROMO_COPY.map(item => (
                <div key={item.platform} className="bg-white rounded-xl border border-border/60 overflow-hidden">
                  <button className="w-full flex items-center justify-between p-3 hover:bg-gray-50/50 transition-colors" onClick={() => setExpandedPromo(expandedPromo === item.platform ? null : item.platform)} data-testid={`promo-toggle-${item.platform.toLowerCase()}`}>
                    <div className="flex items-center gap-2.5">
                  ...
                  {expandedPromo === item.platform && (
                    <div className="px-4 pb-4">
                      <pre className="text-xs text-muted-foreground whitespace-pre-wrap font-sans leading-relaxed bg-gray-50 rounded-lg p-3 border border-border/40">{item.copy}</pre>
                    </div>
                  )}
                </div>
              ))}
        ...
        <TabsContent value="templates" className="mt-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {TEMPLATES.map(t => (
              <div key={t.id} className="bg-white rounded-xl border border-border/60 p-4 hover:shadow-sm hover:border-primary/20 transition-all cursor-pointer group" onClick={() => applyTemplate(t)} data-testid={`template-${t.id}`}>
        ...
        <TabsContent value="audience" className="mt-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {enrichedSegments.map(s => (
              <div key={s.id} className="bg-white rounded-xl border border-border/60 p-4" data-testid={`segment-${s.id}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-sm">{s.label}</span>
                  <span className="text-xl font-bold text-primary">{s.count}</span>
                </div>
                <MiniProgress value={s.count} max={userCounts?.all || 1} color="bg-primary/60" />
                <p className="text-[11px] text-muted-foreground mt-1.5">
                  {userCounts?.all ? `${((s.count / userCounts.all) * 100).toFixed(1)}% of total` : "—"}
                </p>
              </div>
            ))}
          </div>
      ...
      <Dialog open={showModal} onOpenChange={v => { if (!v) resetModal(); }}>
        <DialogContent className="w-[calc(100vw-2rem)] max-w-3xl mx-auto rounded-xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Campaign editor</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              {!editingId && (
                <>
                  ...
                  <div className={`flex items-center gap-2 p-2.5 rounded-lg text-xs border ${form.watch("scheduleType") === "now" ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-blue-50 border-blue-200 text-blue-800"}`}>
                    ...
                    {form.watch("scheduleType") === "now" ? `Sends immediately to ~${audienceCount} users with email marketing enabled.` : `Will be scheduled for ~${audienceCount} users.`}
                  </div>
                </>
              )}
              ...
              <DialogFooter className="gap-2 pt-2">
                ...
                <Button type="submit" size="sm" disabled={createMutation.isPending || updateMutation.isPending || sendMutation.isPending} data-testid="button-submit-campaign">
                  {(createMutation.isPending || updateMutation.isPending) ? <><Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> Saving…</>
                    : editingId ? "Save Changes"
                    : form.watch("scheduleType") === "now" ? <><Send className="w-3.5 h-3.5 mr-1.5" /> Create &amp; Send</>
        ...
        <DialogContent className="w-[calc(100vw-2rem)] max-w-md mx-auto rounded-xl">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className={`flex items-start gap-2 p-3 rounded-lg border text-xs ${recipientPreview === null ? "bg-gray-50 border-gray-200 text-gray-600" : forceSend ? "bg-green-50 border-green-200 text-green-800" : recipientPreview.eligible === 0 ? "bg-red-50 border-red-200 text-red-800" : "bg-blue-50 border-blue-200 text-blue-800"}`}>
              {recipientPreview === null ? <><Loader2 className="w-4 h-4 shrink-0 animate-spin mt-0.5" /><span>Checking…</span></>
                : <><Mail className="w-4 h-4 shrink-0 mt-0.5" /><span>{forceSend ? <><strong>{recipientPreview.total}</strong> recipients (override active).</> : recipientPreview.eligible === 0 ? <>0 recipients — enable override below.</> : <><strong>{recipientPreview.eligible} of {recipientPreview.total}</strong> users have marketing enabled.</>}</span></>}
            </div>
            {recipientPreview && recipientPreview.total > recipientPreview.eligible && (
              <label className="flex items-center gap-2.5 cursor-pointer select-none p-3 rounded-lg border border-amber-200 bg-amber-50 text-xs text-amber-900">
                ...
                <span><strong>Override preferences</strong> — include opted-out users ({recipientPreview.total - recipientPreview.eligible} extra)</span>
              </label>
            )}
          </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-muted/50 rounded-lg p-3 text-xs space-y-1.5">
                  ...
                  <p><span className="text-muted-foreground">Audience: </span><strong>{AUDIENCE_LABELS[viewCampaign.targetAudience]}</strong></p>
